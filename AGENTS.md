...
- Only the `grok` CLI (Designer) edits `scad/*.scad`, locally or via `scripts/invoke-grok-build.sh` on the Bot computer. Grok Bot must not type `.scad` by hand.
- Only Grok Bot writes `review/REVIEW.md` and PNGs under `renders/`.
